...
/* DONE */ FoliationRP2::SeparatrixSegment::SeparatrixSegment(const std::vector<CirclePoint>& DivPoints) :
    ...
    m_Side(CENTER),
    m_IndexOfContainingInterval(THE_POINT_IS_A_DIVPOINT)
{
}
...
/* DONE */ void FoliationRP2::ShiftToSide(SeparatrixSegment& s, LeftOrRight Side){
    ...
    s.m_Side = Side;
    s.m_Endpoint = s.m_Endpoint.ShiftTo(s.EndpointShiftWhenBeginShiftedTo(Side));
    s.m_IntervalIntersectionCount[s.m_Side == LEFT ? DecreaseIndex(s.m_Separatrix) : s.m_Separatrix]++;
    if (s.EndsOnFrontSideQ() && s.m_Depth > 1) {
        assert(s.m_IndexOfContainingInterval != THE_POINT_IS_A_DIVPOINT);
        s.m_IntervalIntersectionCount[s.m_IndexOfContainingInterval]++;
    }
    if (s.m_Depth > 1) {
        s.m_ArcsAroundDivPoints.InsertPoint(m_DivPoints[s.m_Separatrix].ShiftTo(Side));
    }
}
...
/* DONE */ void FoliationRP2::AddToGoodSegmentsIfGood(const SeparatrixSegment& s){
    if (s.m_ArcsAroundDivPoints.ContainsQ(s.m_Endpoint)){
        for (LeftOrRight Side = LEFT; Side < 2; Side++) {
            ...
            SeparatrixSegment& SavedSegment = m_GoodShiftedSeparatrixSegments[s.m_Separatrix][Side].back();
            ShiftToSide(SavedSegment, Side);
            if (!SavedSegment.m_ArcsAroundDivPoints.ContainsQ(SavedSegment.m_Endpoint))
...
/* DONE */ void FoliationRP2::Lengthen(int SeparatrixIndex){
    ...
    s.m_Endpoint = s.EndsOnFrontSideQ() ? s.m_Endpoint.Antipodal() : FollowTheLeaf(s.m_Endpoint, s.m_IndexOfContainingInterval);
    s.m_Depth++;
    s.m_IndexOfContainingInterval = WhichIntervalContainsNew(s.m_Endpoint);
    
    // Checking if the new endpoint is a division point in which case we reached a saddle connection
    if (s.m_IndexOfContainingInterval == THE_POINT_IS_A_DIVPOINT) {
...
/* DONE */ FoliationRP2::SeparatrixSegment& FoliationRP2::GetFirstIntersection(int SeparatrixIndex, const Arc& arc, const LeftOrRight ReturnSide){
    ...
    while (!arc.ContainsQ(m_GoodShiftedSeparatrixSegments[SeparatrixIndex][ReturnSide].back().m_Endpoint.ShiftTo(CENTER))) {
        Lengthen(SeparatrixIndex);
...
FoliationRP2::TransitionData FoliationRP2::CutAlongCurve(const GoodOneSidedCurve& GoodCurve){
    ...
    for (int i = 0; i < m_NumSeparatrices; i++) {
        ...
        if (DivPoints[i].GetPosition() < 0.5) {
    ...
    for (int ShortIntervalIndex = 0; ShortIntervalIndex < m_NumSeparatrices; ShortIntervalIndex += 2) {
        ...
        } else
        {
            ...
            Arc LengthenedArc(GoodCurve.m_ConnectingArc.GetLeftEndpoint(), GoodCurve.m_ConnectingArc.GetRightEndpoint().ShiftTo(CENTER));
