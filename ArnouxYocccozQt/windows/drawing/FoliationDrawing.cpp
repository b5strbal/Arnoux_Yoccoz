...
void FoliationDrawing::setPermutationLabels(bool visible)
{
    permutationLabelsShown = visible;
    update();
}

void FoliationDrawing::setLengthsLabels(bool visible)
{
    lengthsLabelsShown = visible;
    update();
}

void FoliationDrawing::setColoredFilling(bool visible)
{
    coloredFillingShown = visible;
    update();
}

void FoliationDrawing::setTouchingSepSegments(bool visible)
{
    touchingSepSegmentsShown = visible;
    update();
}

void FoliationDrawing::drawSepSegment(const balazs::SeparatrixSegment *pSegment)
{
    pSepSegment = pSegment;
    update();
}

void FoliationDrawing::drawTransverseCurve(const balazs::TransverseCurve *ptc)
{
...
void paintFilling(const balazs::Foliation& foliation, QPainter &painter, int folW, int folH)
{
    ...
    if(foliation.numIntervals() == 1){
        QBrush brush(Qt::GlobalColor(7), Qt::Dense4Pattern);
        painter.fillRect(QRectF(0, 0, folW, folH), brush);
        painter.fillRect(QRectF(0, folH, folW, 0.05 * folH), brush);
        return;
    }
