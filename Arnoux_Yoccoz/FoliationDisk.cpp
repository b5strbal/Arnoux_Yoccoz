...
void FoliationDisk::Init(){
    ...
    for (int i = 0; i < m_NumSeparatrices; i++) {
        if (m_Lengths[i] < 3 * PRECISION) {
            throw "The lengths parameters of the foliation are too small.";
        }
    }
    ...
    m_ShiftedDivPoints.resize(2 * m_NumSeparatrices);
    m_ShiftedDivPoints[0] = PRECISION;
    m_ShiftedDivPoints[2 * m_NumSeparatrices - 1] = 1 - PRECISION;
    for (int i = 1; i < m_NumSeparatrices; i++) {
        m_ShiftedDivPoints[2 * i - 1] = m_DivPoints[i].GetPosition() - PRECISION;
        m_ShiftedDivPoints[2 * i] = m_DivPoints[i].GetPosition() + PRECISION;
    }
    m_TempForFollowTheLeaf.resize(m_NumSeparatrices);
    for (int i = 0; i < m_NumSeparatrices; i++) {
        m_TempForFollowTheLeaf[i] = m_DivPoints[m_Pair[i]].GetPosition() + m_DivPoints[IncreaseIndex(i)].GetPosition();
    }
}
...
CirclePoint FoliationDisk::FollowTheLeaf(const CirclePoint& t){
    return FollowTheLeaf(t, WhichIntervalContainsNew(t));
}

CirclePoint FoliationDisk::FollowTheLeaf(const CirclePoint& t, int IndexOfContainingInterval){
    assert(IndexOfContainingInterval != THE_POINT_IS_A_DIVPOINT);
    return -t + m_TempForFollowTheLeaf[IndexOfContainingInterval];
}
...
int FoliationDisk::FindBetweenShiftedDivPoints(double d, int StartIndex, int EndIndex){
    if (StartIndex == EndIndex) {
        return StartIndex;
    }
    int IndexOfMiddleShiftedDivPoint = (StartIndex + EndIndex - 1)/2;
    if (m_ShiftedDivPoints[IndexOfMiddleShiftedDivPoint] < d) {
        return FindBetweenShiftedDivPoints(d, IndexOfMiddleShiftedDivPoint + 1, EndIndex);
    } else
        return FindBetweenShiftedDivPoints(d, StartIndex, IndexOfMiddleShiftedDivPoint);
}


int FoliationDisk::WhichIntervalContainsNew(const CirclePoint& t){
    int Index = FindBetweenShiftedDivPoints(t.GetPosition(), 0, 2 * m_NumSeparatrices);
    if (Index % 2 == 1) {
        return Index/2;
    } else
        return THE_POINT_IS_A_DIVPOINT;
}
